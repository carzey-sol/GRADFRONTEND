    ...
    const fetchCoursesForTeacher = async () => {
      ...
      try {
        const teacherResponse = await axios.get(
          `http://localhost:4000/api/viewCourseMaterials/students?email=${email}`
        );
        const courseId = teacherResponse.data.course_id;

        const coursesResponse = await axios.get(
          `http://localhost:4000/api/viewCourseMaterials/courses?course_id=${courseId}`
        );
        setCourses(coursesResponse.data);

        const subjectsResponse = await axios.get(
          `http://localhost:4000/api/viewCourseMaterials/subjects?course_id=${courseId}`
        );
